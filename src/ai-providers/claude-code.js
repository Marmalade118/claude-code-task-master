...
export async function generateClaudeCodeText({
	...
	const promptContent = systemMessage
		? `${systemMessage}\n\n${userMessage}`
		: userMessage;
	...
	return new Promise((resolve, reject) => {
		...
		claudeProcess.on('close', (code) => {
			...
			if (code === 0) {
				...
				reject(
					new Error(`Claude Code CLI failed with code ${code}: ${stderr}`)
				);
			}
		});
...
export async function generateClaudeCodeObject({
	...
	while (attempts < maxRetries) {
		...
		try {
			...
			log(
				'debug',
				`Raw response from Claude (attempt ${attempts}): ${jsonText.substring(0, 500)}...`
			);

			// Write full response to temp file for debugging
			const debugFile = path.join(
				os.tmpdir(),
				`claude-response-${uuidv4()}.txt`
			);
			...
			try {
				...
				if (!jsonMatch) {
					...
					log(
						'error',
						`Could not find JSON in response. First 200 chars: ${cleanedText.substring(0, 200)}`
					);
					log(
						'error',
						`Last 200 chars: ${cleanedText.substring(Math.max(0, cleanedText.length - 200))}`
					);
					...
					try {
						...
						if (errorMatch) {
							...
							if (position > fixedJson.length - 100) {
								log(
									'error',
									'Error occurs near end of response - likely truncated'
								);
								throw new Error(
									'Response appears to be truncated. Try reducing --num-tasks or simplifying the PRD.'
								);
						...
						log(
							'error',
							`Failed to parse even after fixes. JSON fragment: ${fixedJson.substring(0, 200)}`
						);
						log(
							'error',
							`Full response length: ${jsonText?.length || 0} characters`
						);
			...
			if (attempts === maxRetries) {
				log(
					'error',
					`All attempts failed. Last raw response: ${jsonText?.substring(0, 1000) || 'No response'}`
				);
			}
	...
	throw new Error(
		`Failed to generate valid object after ${maxRetries} attempts: ${lastError.message}`
	);
}
